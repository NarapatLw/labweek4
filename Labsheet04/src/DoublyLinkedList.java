...
public class DoublyLinkedList {
	...
	// Method for inserting at the end of list เพิ่มข้อมูลต่อท้ายลิสต์
	public void insert(Object value) {
		Node new_node = new Node(value); //if case empty list
	...
	public void insert(int position, Object value) {
		...
		if (head == null) {
			head = new_node;
			tail = new_node; //same as 1 paramiter
		} else if (position==0) {
			new_node.next = head; 
	...
	public void remove() { //ลบที่ไม่มีพารามิเตอร์ หรือลบตัวสุดท้าย
		if (head != null) { //if case note empty list
			if (head.next == null) {
			head=null;
			tail=null;
		} else {
